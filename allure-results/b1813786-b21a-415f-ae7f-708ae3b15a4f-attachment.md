...
```ts
  ...
  51 |     const dialog =await waitforalert
  52 |     const alertMessage = await loginpage.handleAlert(dialog)
  53 |     console.log("Alert message in test:", alertMessage)
> 54 |     await expect(alertMessage).toContainText(' Wrong Password.')
     |                                ^ Error: toContainText can be only used with Locator object, was called with Wrong password.
  55 |  
  56 | }
  57 | )
