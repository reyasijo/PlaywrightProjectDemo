...
```ts
  ...
  66 | //add assertion for title after logout
  67 |  //const title=await loginpage.logout()
> 68 |  console.log("Title after logout is:", title)
     |                                        ^ ReferenceError: title is not defined
  69 |  await expect(title).toBe('STORE')
  70 | 
  71 | })
  72 | 
  73 | 
