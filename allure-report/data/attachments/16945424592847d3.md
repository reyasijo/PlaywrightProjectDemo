...
```
ReferenceError: waitforalert is not defined
```
...
```ts
  ...
  15 |         await this.loginbtn2.click()
  16 | 
  17 |     }
  18 | 
  19 |      async handleAlert()
  20 |     {
> 21 |         const dialog = await waitforalert() 
     |                        ^ ReferenceError: waitforalert is not defined
  22 |         const dialogemsg=dialog.message()
  23 |         console.log("Alert message is:" + dialogemsg)
  24 |         expect(dialog.message()).toBe('User doews not exist')
