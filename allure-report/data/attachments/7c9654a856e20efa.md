...
- Location: tests\Logintest.spec.js:15:6
...
```ts
  ...
  8  | 
  9  |     signupPage = new SignupPage(page)
  10 | 
  11 |     await signupPage.goto()
  12 | 
  13 | })
  14 | 
  15 | test.only('sign up as a new user', async({page})=>
  16 | {
  17 |    // signuppage=new SignupPage(page)
  18 |    // await signuppage.goto()
> 19 |     await signuppage.signUpEnterCreds(userData.signupusername,userData.signuppassword)
     |     ^ ReferenceError: signuppage is not defined
  20 |     //Handling the alert on Signup action
  21 |     const waitforalert = page.waitForEvent('dialog') //waitis for alert
  22 |     await signuppage.signUp()
  23 |     const dialog = await waitforalert //wait until alert appears
  24 |     console.log("Alert message is:", dialog.message())
  25 |     expect(dialog.message()).toBe('Sign up successful.')
  26 |     await dialog.accept()
  27 | })
  28 | 
  29 | test('sign up enter creds and close',async({page})=>
  30 | {
  31 |     const signuppage=new LoginPage(page)
  32 |     await signuppage.goto()
  33 |     await signuppage.signUpEnterCreds(userData.username,userData.password)
  34 |     await signuppage.signUpClose()
  35 | }
  36 | )
  37 | test('login using valid credentials', async({page})=>
  38 | {
  39 | 
  40 | }
  41 | )
